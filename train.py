...
def _main(args):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=args.exp.batch_size, num_workers=0, pin_memory=True, worker_init_fn=setup.worker_init_fn,timeout=0)
    train_loader=iter(train_loader)

    test_set=hydra.utils.instantiate(args.dset.test)
    test_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=1,  num_workers=0, pin_memory=True, worker_init_fn=setup.worker_init_fn)
